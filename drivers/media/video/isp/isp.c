		  ...
		  void *priv)
{
	...
	if (ISP_BUFS_IS_EMPTY(bufs)) {
		 ...
		 */
		if (isp->config->u.csi.use_mem_read) {
//			bufs->wait_hs_vs = 0;  //NCB-TI CSR OMAPS00228014 Fix
			/*
...
static int isp_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i <= OMAP3_ISP_IOMEM_CSI2PHY2; i++) {
		...
		printk("  isp->mmio_base_phys[i] = %x\n", isp->mmio_base_phys[i]);
		printk("  isp->mmio_size[i] = %x\n",isp->mmio_size[i] );
	...
	printk(" camera irq_number = %x\n", isp->irq_num );


	isp->mclk = CM_CAM_MCLK_HZ / 2;  //192 mz
